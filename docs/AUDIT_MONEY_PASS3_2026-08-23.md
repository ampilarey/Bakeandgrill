...
Read-only — **nothing in this report has been changed.** Audited against
`main` at `53284d79c`. Same caveat as before: this was traced by reading, with
assumptions checked against schema and callers where they mattered, but not by
attacking a running system. The first audit's own error rate (two findings
wrong in some respect) is the reason every remedy below says *verify before
fixing*.
...
**Severity: Medium — no money is lost, but a cashier can be blamed for a shortage that isn't theirs**
...
**Severity: Medium if Xero is relied on; cosmetic otherwise**
...
**Severity: Medium if Xero is used for anything tax-adjacent; informational otherwise**
...
**Severity: Low-Medium** · `app/Domains/Catering/Services/CateringQuoteService.php:124-146`,
...
**Severity: Low** · `app/Domains/Payments/Services/PaymentCommissionService.php:135-140`,
...
**Severity: Low, conditional on Stripe use** · `PaymentCommissionService.php:21-24,49-63`
...
**Severity: Low, latent** · `ShiftController.php:72-76`
...
**Severity: Low** · `app/Domains/Deposits/Services/DepositLedgerService.php:337-359`
...
**Severity: Low, latent** · `DepositLedgerService.php:390-392`
