...
**FINDING (§7.2 text-hug):** Paint must live on an inner inline `.hero-text-bg` (flex items blockify the heading into one rectangle). Do **not** split on `<br>` into `display:block` pills — soft-wrap inside each pill still makes wide rectangles. Do **not** open `line-height` to avoid clone overlap (that spreads lines and shifts the copy block). Correct approach: one inline span + `box-decoration-break: clone`, **horizontal pad only** (`padding: 0 0.4em`) so strips hug each visual line at the normal title spacing. Full-width bar still paints the heading.
