...
**FINDING (§7.2 text-hug):** Paint must live on an inner inline `.hero-text-bg` (flex items blockify the heading into one rectangle). Do **not** split on `<br>` into `display:block` pills. Do **not** open `line-height` to avoid overlap. Do **not** set inline `background:` on the hug span (shorthand overrides stylesheet). Correct approach: one inline span + `box-decoration-break: clone`, horizontal pad, and a band via `background-size: 100% 0.93em` positioned `left 0.15em` (biased down to cover baselines without empty pad above glyphs; hairline gap vs next line). Full-width bar still paints the heading.
