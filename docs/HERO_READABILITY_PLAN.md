...
**FINDING (§7.2 text-hug):** Putting `display: inline` + background on the title/subtitle heading itself failed inside the flex `.banner-copy` / `.home-promo-hero__copy` column — flex items are blockified, so the background became one big rectangle. Fix: paint text-hug backgrounds on an inner `.hero-text-bg` span; full-width bar still paints the heading. With `box-decoration-break: clone`, vertical padding + the default title `line-height: 1.08` stacked per-line backgrounds — open line-height via `data-bg-hug="1"` (`1.4` / `1.7`) and keep pad mostly horizontal (`0.05em 0.4em`).
