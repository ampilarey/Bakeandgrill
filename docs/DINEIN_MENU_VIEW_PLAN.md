...
**Goal:** A separate, **view-only** menu page for dine-in customers — categories, items, offers/
discounts, and new items — with **no login, cart, ordering, or app chrome**. Shareable as a **QR code**
and printable. Modern, mobile-friendly, attractive (reuses the ZUS card look). Built as a **standalone
React route** reusing the existing public API and components; "New" is derived from `created_at`.
