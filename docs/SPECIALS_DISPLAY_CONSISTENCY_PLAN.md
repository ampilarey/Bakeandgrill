...
Status: **Unverified — needs a look.** Specials/offers surfaces exist on website and order app, but this short scan could not prove every consistency fix in the plan landed. Re-verify against live specials cards/carousels before treating as done or re-implementing.

> Rescued from branch `claude/specials-display-consistency-plan` (not written fresh on this branch).
