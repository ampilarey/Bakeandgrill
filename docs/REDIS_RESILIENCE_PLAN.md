...
Status: **Partly built — full dual-store / RedisException failover status unverified — needs a look.** `CACHE_LIMITER_STORE` / the `limiter` cache store exists in `config/cache.php` and is used for throttling. Broader plan items (dedicated RedisException fallbacks, dual Redis, monitoring) were not clearly found in a short scan — do not assume the full plan shipped.

> Rescued from branch `claude/redis-resilience-plan` (not written fresh on this branch).
