...
> Date: 2026-07-20 (rev 12: + duplicated custom-price review flag — design frozen, reviewer verdict READY) · Status: approved design, ready for implementation
...
### Event ownership & accountability (binding)

- **Every event has an accountable handler** via the existing `catering_requests.handled_by` column, promoted to a first-class concept. Lifecycle: customer-created events start unassigned → auto-assigned to the first staff member who takes a meaningful action (edit lines, change status — the controller's existing auto-assign behavior is kept) → **"Send quote" auto-assigns the sender when still unassigned**, so no event reaches the commercial stage without an owner. Admin can assign/reassign explicitly via a dropdown of `events.manage` holders.
- **Ownership = accountability, NOT lockout**: any `events.manage` holder can act on any event (cover for shifts/sick days); the handler is who is answerable. Never restrict actions to the handler.
- **Handler-aware notification targeting** (refines the recipient helper): UNASSIGNED events — and the initial "event created" notification — go to ALL `events.manage` holders + the fallback `catering_notify_phone`/`catering_notify_email` settings (cast wide so someone picks it up). ASSIGNED events — subsequent lifecycle notifications (quote sent, approved & paid, expired, cancelled, reminder) go to the handler + the fallback settings only. Customer notifications are unaffected.
- **Visibility**: the handler's name shows on the admin pipeline list + detail page and on the POS Events tab card ("Handled by: …"); the pipeline list gets an "assigned to me" filter.
- **Created by / last updated by**: creator is the linked customer (`customer_id`, added in Phase 2 — see below) or the staff member for phone-in events; "last updated by" is NOT a column — every catering edit/send/assignment writes to the existing audit log, which is the system of record for who did what.
...
### Event fulfilment information (V1 spec — three tiers)

**Tier 1 — dedicated columns on `catering_requests`** (structured; queried, displayed, printed):
`fulfillment_method` (pickup|delivery), `event_date` (exists), `event_time`/`fulfillment_time` (exists), **`setup_time`** (when food must be on site/ready — what the kitchen plans around, distinct from event start), `venue_name`, `delivery_address` (+ `delivery_island` since delivery is island-aware; both required only when method=delivery), `onsite_contact_name`, `onsite_contact_phone` (default to the booker, editable), `headcount` (exists), **`dietary_notes`** (allergies + dietary requirements — first-class text, NEVER buried in general notes; food-safety field that must print on the kitchen chit and show prominently on the POS event card).

**Tier 2 — one `fulfilment_options` JSON column** for operational flags (staff-edited checkbox group in the admin editor; extensible without migrations): `setup_required`, `service_type` (food_only|full_setup), `disposables_needed`, `serving_equipment`, `staff_onsite`. The customer wizard does NOT ask these — staff capture them during the quoting conversation.

**Tier 3 — free text** (exists): customer notes (wizard) and internal staff notes (admin only, never shown to the customer).

Two binding consequences:
- **Delivery fee for events is a quote LINE, not the engine.** The zone-based `DeliveryFeeCalculator` does NOT apply to catering-type orders — staff price delivery/setup as a normal quote line ("Delivery & setup — MVR X") so it is visible, negotiable, and inside the locked quote.
- **Fulfilment info travels to every acting surface**: the public quote page (customer confirms venue/time before paying), confirmation SMS/email (date, time, venue), the POS Events tab card (venue, setup time, headcount, dietary badge), and the kitchen chit at fire-to-kitchen (setup time + dietary notes prominently).

### Quote price-lock rule (binding — a sent quote is the commercial agreement)

- **Prices lock the moment staff tap "Send to customer".** From then until expiry, the quote's line prices are fixed. Catalog/menu price changes NEVER reprice a sent quote, and NEVER block its approval. Example that must hold end-to-end: Monday staff quote a Fried Rice Tray at MVR 450 → Tuesday the menu price changes to MVR 500 → Wednesday the customer approves and **pays MVR 450**.
- **The order at approval is created FROM the quote snapshot** (`catering_request_lines` prices), never from current catalog prices. Trust model intact: the snapshot is server-persisted and staff-sent; the customer submits only the token. This is the same controlled exception as staff-priced custom lines.
- **Only two things change a sent quote's prices**: staff edit + resend (which re-resolves prices at edit time and rotates the link, so the customer always approves exactly what they see), or expiry (after which nothing can be approved — expiry, `catering_quote_valid_days` + the pre-event cutoff, is THE staleness control; there is no price-drift guard).
- **Item deactivated/86'd/deleted after sending**: approval still succeeds — lines carry snapshot name + price; a line whose item record no longer exists falls back to the custom-item placeholder with the snapshot name/price.
- Caveat (document on receipts logic, do not "fix"): GST is applied at the statutory rate in force at order creation — a government rate change between quote and approval may shift the tax line; that is regulatory, not repricing.
...
### Repeat ordering (V1: staff duplicate; V2: customer "Book again")

Catering customers reorder — the structured `catering_request_lines` + `customer_id` architecture supports this with zero extra schema.

- **V1 — staff "Duplicate event"** (Phase 3): an action on the admin detail page creating a new `draft` (new reference, no token, unassigned — ownership rules then apply) copying: all lines — **catalog lines RE-RESOLVE to current catalog prices**; custom lines carry name/qty/price as an editable starting point — and the admin editor must visually flag these inherited custom prices as HISTORICAL defaults needing review (e.g. amber "copied price — review" badge that clears once the price field is touched or confirmed), so an old negotiated price is never reused unnoticed — plus fulfillment fields (method, venue, address/island, contacts, dietary notes, ops flags) **except date/times, which must be entered fresh**.
- **PRICING RULE (binding)**: the rev-6 price lock applies to a SENT quote only. A duplicate is a new draft at TODAY'S prices — duplication must never resurrect old pricing, or it would quietly defeat the price lock and quote expiry.
- **V1 — pipeline search** (Phase 3): the admin pipeline list is searchable by customer name/phone so staff pull a repeat customer's history in one step.
- **V2 — customer "Book again" (explicitly deferred, mechanism named)**: a button on a My-events row opening the wizard prefilled from that event's lines via a `?from={reference}` parameter (generalization of the rev-5 `?add={itemId}` preselect). No schema/API redesign needed later — purely a wizard-prefill feature.
...
## 5. Reporting behaviour & future extension points (documentation — no V1 tasks beyond the two tweaks noted)

**Custom items in reports.** All custom quote lines use the single seeded "Custom catering item" placeholder, which lives in a dedicated **"Custom Catering" category** (seeding requirement — see Phase 4). Consequences, by design: item-level sales reports show ONE aggregate row for all custom revenue — instantly distinguishable from standard menu items; category-level reports group it under Custom Catering; the staff-entered names ARE preserved on each order line, so receipts, order detail, and the audit trail always show the real text ("Wedding cake — 3 tiers"). A per-name breakdown report (group order lines where item = placeholder) is a named V2 report, not built in V1. Custom lines are taxed at the standard GST rate by default (the existing empty-tax-code → Standard8 fallback) — staff should price them accordingly.

**Calendar view (deferred, architecture-ready).** Events carry `event_date` + event/setup times, indexed on `(event_date, status)` (Phase 2 migration), and the Phase-5 POS events endpoint accepts date-range filters. Day/week/month calendar views are therefore pure frontend rendering over the SAME endpoint — no backend work when wanted. Deferral is intentional.

**Kitchen capacity planning (out of V1 scope, room reserved).** A future capacity module is aggregate queries over confirmed events per date (count, total headcount, optionally line volumes) compared against configurable limits (max events/day, max headcount/day). The natural hook point is the **send-quote endpoint**: a soft, non-blocking staff warning ("Saturday already has 5 events / 400 heads") before committing to a new quote. Capacity must WARN, never auto-block — the judgment stays human. No schema is needed now; events already carry everything the queries require.

**Operational dashboards (extension point).** Every anticipated dashboard — upcoming events this week, events by status, awaiting approval/deposit/kitchen-release, today's deliveries, weekly headcount — is a simple aggregate over `catering_requests` (+ linked order payment state). Catering **revenue trends come free** from the existing reports pipeline because event revenue flows through normal `catering`-type orders (filter by order type). The Phase-5 optional dashboard widget is the first instance of this pattern; the rest are additive queries with no schema changes.
...
TASKS:
1. Migration: `catering_request_lines` — id, catering_request_id FK cascade, item_id nullable FK (null = custom line), variant_id nullable, name (snapshot or custom text, ≤160), quantity int ≥1, unit_price decimal(10,2) nullable (null until priced; catalog lines get server-resolved price snapshots), notes ≤500, is_custom bool, sort_order, timestamps. Add to catering_requests (fulfilment spec, section 2 "Event fulfilment information"): `customer_id` nullable FK → customers nullOnDelete (set from the authenticated customer creating the draft — REQUIRED for the Phase-4 "My events" list and logged-in flows; the original table never had it), `event_type` string nullable, `fulfillment_method` string default 'pickup' (pickup|delivery), `fulfillment_time` time nullable, `setup_time` time nullable, `venue_name` ≤160 nullable, `delivery_address` ≤500 nullable, `delivery_island` ≤80 nullable, `onsite_contact_name` ≤120 nullable, `onsite_contact_phone` ≤32 nullable, `dietary_notes` ≤1000 nullable, `fulfilment_options` JSON nullable — plus keep all existing columns, and add an index on `(event_date, status)` (calendar/capacity queries, section 5). Validation: delivery_address + delivery_island required when fulfillment_method=delivery.
...
6. Wizard (apps/online-order-web): resurrect PreOrderPage.tsx as EventOrderPage at route /order/events (also point /order/pre-order there): step 1 item picker with THREE TABS — "Catering menu" (default), "Regular menu", "+ Custom item" (name/qty/note rows) — so catering leads and the regular menu is one tap away (inverse of the main ordering app); step 2 event details per the fulfilment spec: pickup/delivery toggle, date (honoring the lead-time min), event time, venue name + delivery address/island (shown + required only for delivery), on-site contact name/phone (prefilled from the booker, editable), occasion, headcount, dietary/allergy field (its own labelled input, not folded into notes), customer notes — the Tier-2 ops flags are NOT asked here (staff capture them); step 3 OTP-or-authenticated confirm showing an itemized summary + the fulfilment summary (method, date/time, venue) — catalog lines with prices, custom lines marked "to be quoted", and copy that staff will confirm the final quote; step 4 done screen with the reference number and "we'll SMS/email you the quote". Keep the existing CateringPage simple-inquiry form reachable (link from the wizard: "just want a callback?").
...
2. NEW PERMISSION `events.manage`: add to the permissions seeder/catalog alongside existing slugs (reservations.manage etc.), assignable through the existing roles/permissions admin UI — this is how the admin "appoints" event staff. Gate ALL new endpoints in this phase (and later phases) with events.manage instead of customers.manage (existing catering list/update endpoints keep their current gate for backward compatibility). Recipient helper (HANDLER-AWARE, section 2 ownership rule): a small service returning notify targets — when the event is unassigned (handled_by null) or for the "created" notification: all active users holding events.manage (phone + email) + fallback site settings catering_notify_phone / catering_notify_email; when assigned: the handler + the fallback settings only. Refactor the Phase 2 notifier to use it. Ownership mechanics: keep the existing auto-assign-on-first-staff-action behavior; send-quote (task 4) auto-assigns the sender when handled_by is null; add an explicit assignee dropdown (events.manage holders) in the admin editor and an "assigned to me" filter on the pipeline list; every edit/send/assignment writes to the existing audit log.
...
5. Admin UI (CateringPage or a detail drawer/page): line editor (qty steppers, remove, add catalog item via item search, price inputs enabled only on custom lines), quote summary (subtotal, tax preview, total), payment amount input + "deposit" toggle with validation, Send/Resend button showing version + expiry, and a status timeline. Fulfilment editor (section 2 spec): all Tier-1 fields editable (method, date, event/setup times, venue, address+island, on-site contact, headcount, dietary notes — dietary visually prominent) plus the Tier-2 `fulfilment_options` checkbox group (setup required, food-only vs full setup, disposables, serving equipment, staff on-site). Sending is blocked while method=delivery with an empty address (in addition to the unpriced-custom-lines block). Delivery/setup cost is priced as a normal quote LINE — the delivery-fee engine is never invoked for events. Keep all existing pipeline fields (staff_notes, pos_order_id, handled_by) working.
...
6b. Repeat ordering (section 2 rule): POST /admin/customers/catering-requests/{id}/duplicate (events.manage) → new draft per the section-2 duplication semantics (catalog lines re-priced to CURRENT catalog — never copy old prices for catalog lines; custom lines copied editable; fulfillment copied except date/times; new reference, no token, unassigned) + a "Duplicate event" button on the admin detail page. Pipeline list gains customer name/phone search (server-side filter param).
...
TESTS: feature — line replace re-resolves catalog prices (client prices ignored), custom price requires events.manage, send blocked with unpriced custom lines, deposit bounds validation, resend rotates token + old token dies + notifies (idempotent), expiry computation (both min branches), public endpoint token/expiry behavior, events.manage permission gating (403 without it), OWNERSHIP: send-quote auto-assigns the sender when unassigned; unassigned events notify all events.manage holders while assigned events notify only the handler (+fallback settings); explicit reassign round-trips; assignment writes an audit entry. DUPLICATE: catalog lines re-priced to current catalog after a price change (never the old snapshot), custom lines copied, date/times cleared, new reference, unassigned, no token; pipeline search filters by name/phone. Vitest — admin editor interactions incl. assignee dropdown, "assigned to me" filter, duplicate button (copied custom lines render the historical-price review badge; badge clears on edit/confirm), pipeline search. Full suites green.
...
TASKS:
1. Quote page (apps/online-order-web, route /order/quote/{token}, public): renders the Phase-3 GET /api/event-quotes/{token} — itemized lines, tax, total, "Pay now: MVR X (deposit)" or full, expiry countdown, PLUS the fulfilment summary (pickup/delivery, event date + time, setup time, venue/address, on-site contact, headcount, dietary notes) so the customer confirms the logistics before paying, then the Approve & Pay button; expired/invalid states with a contact link. Confirmation SMS/email (task 3) must include date, time, and venue.
2. Approval endpoint: POST /api/event-quotes/{token}/approve (public, throttle 10/min). Server-side, atomically (row lock on the catering request): validate token live + not expired + status awaiting_customer; create an Order via OrderCreationService with a new type `catering` (add to order-type enums/validation; channelForOrderType('catering') already maps to online_pickup — keep that). PRICE-LOCK RULE (section 2 — binding): ALL order lines are priced FROM the catering_request_lines snapshots via an internal priced-from-quote creation path — do NOT re-resolve current catalog prices, do NOT compare against them, and there is NO price-mismatch rejection (the old 1-laar/409 idea is explicitly dead — catalog changes never block or reprice a sent quote; expiry is the only staleness control). The priced-from-quote path is internal-only (never reachable from API request input — request validation still accepts no prices). Catalog lines whose item still exists use that item_id with the snapshot unit price; custom lines AND lines whose item record was deleted use the designated "Custom catering item" placeholder (seeded, is_active=false so it never appears in menus, placed in a dedicated "Custom Catering" category so item/category reports aggregate custom revenue distinctly — section 5) with snapshot name + price. Deactivated or 86'd items do NOT block approval. WORKFLOW RULE (section 2): this is a FUTURE-DATED event order — SKIP today's stock/86/kitchen-capacity validation; do NOT deduct stock at approval (stock is handled at fire-to-kitchen, Phase 5); lead-time/quote validation applies instead. Order starts payment_pending, kitchen print suppressed. Store order_id on the catering request (pos_order_id), keep status awaiting_customer until paid. Then create the BML payment for quote_payment_laar (NOT necessarily the order total — deposit case) via the same PaymentService path gift cards use, and return the payment_url for redirect.
...
7. Adoption (maps 4A/4C): admin OrdersPage gains a 'catering' order-type label + type filter (pattern: existing preorder label ~line 52); add the catering_reminder_enabled toggle to the Phase-3 settings card; remove the "ring through POS with a manual discount" hint from CateringPage. Order app: GET /api/customer/event-orders (list, customer token — scoped strictly by catering_requests.customer_id, with a test that a customer can NEVER see another customer's events) and an AccountPage "My events" section listing them (reference/date/status/paid+balance laari), each row linking to /order/quote/{token} while awaiting_customer; replace the old pre-order history links/empty-states with it. DISCOVERABILITY (both required): (a) the app's "Events" navigation entry (added in Phase 2 pointing at the wizard) is extended to expose "My events" for logged-in customers (e.g. submenu or a secondary link) so event history is reachable without digging into Account; (b) the wizard's done screen gains a "View my events" link to that section. Vitest: nav shows My events only when authenticated; done-screen link present.

TESTS: feature — PRICE LOCK end-to-end: quote a line at 450, change the catalog item to 500, approve → order line is 450 and the payment amount is based on 450 (the Monday/Tuesday/Wednesday scenario from section 2); catalog price changes never 409/block approval; deactivated and 86'd items still approve from snapshot; deleted item falls back to placeholder with snapshot name+price; custom placeholder item never menu-visible; DEPOSIT payment confirmation (not OrderPaid) confirms the event exactly once and leaves the order partial; full payment confirms exactly once (no double-notify from PaymentConfirmed + OrderPaid); POS can settle the balance; expiry sweep notifies + reverts idempotently; reminder job; cancellation notifications; expired/rotated token rejected; double-approve idempotent; staff edit blocked while pending payment exists. Vitest — quote page states (live/expired/paid). Full backend + frontend suites green; regenerate contract snapshots if order payloads changed.
...
2. POS "Events" tab alongside Active Orders: events grouped by fulfillment date, Today pinned/highlighted, status chips (awaiting payment / confirmed / in prep / completed / cancelled), each card showing reference, customer, headcount, line summary, paid vs balance amounts, PLUS the fulfilment essentials: pickup/delivery, setup time, venue, "Handled by: {name}" (section 2 ownership rule), and a visually prominent DIETARY badge when dietary_notes is set (tap to read). Visible to all staff at all times — independent of shift state (opening/closing a shift must not affect it). Fire-to-kitchen (task 3a) must print setup time + dietary notes prominently on the kitchen chit.
...
- Named V2 follow-up (do not build in V1): customer "Book again" — wizard prefill via `?from={reference}` from a My-events row (see section 2 "Repeat ordering"). Also deferred: surfacing events in the admin Customer 360 drawer.
