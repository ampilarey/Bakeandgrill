...
Status: **Built** (core platter builder + promotions path live). Models (`PlatterTemplate` / slots / options), admin platter UI, `parent_order_item_id` on order items, `PromotionEvaluator` platter trigger/reward logic, and order-app platter helpers are in the codebase. Treat the body as design history for a shipped feature; re-verify only when extending behaviour.

> Rescued from branch `claude/platter-builder-plan` (not written fresh on this branch).
