...
Status: **Partly built — full plan status unverified — needs a look.** Website footer already has social icons, trust strip, and related chrome in `layout.blade.php`. Menu card display fields were split into a sibling plan and are built. Remaining menu-redesign / mobile UX items in this body were not fully mapped in a short scan.

> Rescued from branch `claude/mobile-menu-footer-plan` (not written fresh on this branch).
