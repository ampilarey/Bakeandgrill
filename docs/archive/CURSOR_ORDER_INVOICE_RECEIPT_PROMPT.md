...
Status: **Historical archive — agent prompt for order/invoice/receipt consistency.** Rescued for history only. Consistency work may already have landed elsewhere.

> Rescued from branch `claude/review-order-invoice-flow-0h8aj` (not written fresh on this branch).
