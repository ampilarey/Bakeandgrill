...
| **NOT COVERED** | Cannot verify trustworthily (skipped with reason). |
...
|---|---|---|---|---|
| 0.1 | Run `./scripts/prod-preflight.sh` on the server | No FAIL lines. `APP_DEBUG` must be false, `APP_ENV` production | **HUMAN ONLY** | `human-only.spec.ts` |
| 0.2 | Run `php artisan app:verify-production-config` | Passes | **HUMAN ONLY** | `human-only.spec.ts` |
| 0.3 | Run `./scripts/post-deploy-smoke.sh test` | All URLs respond | **HUMAN ONLY** | `human-only.spec.ts` |
| 0.4 | Run `./scripts/backup.sh` and confirm a backup file appears | A dated file exists and is not empty | **HUMAN ONLY** | `human-only.spec.ts` |
| 0.5 | Check the deployed commit matches what you merged | Same short SHA | **HUMAN ONLY** | `human-only.spec.ts` |
...
| # | Do this | Should happen | Coverage | Spec |
|---|---|---|---|---|
| 1.1 | Place an online order and pay with a real card | Order paid, receipt SMS arrives, order appears in POS and KDS | **HUMAN ONLY** | `human-only.spec.ts` |
...
| 1.3 | Start a payment, then **close the tab** at the bank page | Order stays unpaid. No half-paid state. Retrying works | **HUMAN ONLY** | `human-only.spec.ts` |
| 1.4 | Start a payment and let it sit 10 minutes before completing | Payment still lands correctly, or fails cleanly with a retry | **HUMAN ONLY** | `human-only.spec.ts` |
| 1.5 | Pay a second order while the first is still open | Both correct, no crossed payments | **NOT COVERED** | — |
| 1.6 | Place a **cash** order in POS and take payment | Drawer total increases by the right amount | **NOT COVERED** | — |
| 1.7 | Give change on a cash order | Change and counted totals are right | **NOT COVERED** | — |
| 1.8 | Pay part cash, part card | Split recorded correctly; both show on the receipt | **NOT COVERED** | — |
| 1.9 | Apply a discount, then pay | GST calculated on the discounted amount, not the original | **NOT COVERED** | — |
...
| # | Do this | Should happen | Coverage | Spec |
|---|---|---|---|---|
| 2.1 | As a **cashier**, request a refund | Goes to pending. No money moves. Owner gets an SMS | **NOT COVERED** | — |
| 2.2 | As the cashier, try to approve **your own** request | Refused | **NOT COVERED** | — |
| 2.3 | As owner, approve it — customer OTP required | Money moves only after the code is entered | **NOT COVERED** | — |
| 2.4 | Check the drawer after a cash refund | Reduced by exactly the refund amount | **NOT COVERED** | — |
| 2.5 | Try to refund an order that was never paid | Refused with a clear message | **NOT COVERED** | — |
| 2.6 | Refund an online order paid by card | Correct handling, customer notified | **NOT COVERED** | — |
...
| 3.3 | Deliberately count wrong, then "Count again" | Both counts recorded. You can see the recount later | **NOT COVERED** | — |
| 3.4 | Close with a difference | Reason required. Shift closes. You get the real numbers | **NOT COVERED** | — |
...
| # | Do this | Should happen | Coverage | Spec |
|---|---|---|---|---|
| 5.1 | Send an order to the kitchen screen | Appears promptly | **NOT COVERED** | — |
| 5.2 | Mark it ready | Customer gets the ready SMS | **HUMAN ONLY** (handset SMS) | `human-only.spec.ts` |
| 5.3 | Log in as a **real cashier account** (not owner) | Can do their job; cannot see reports, settings, wholesale or complaints | **NOT COVERED** | — |
| 5.4 | Try to reach an admin page as that cashier by typing the URL | Blocked | **NOT COVERED** | — |
| 5.5 | Print a receipt on the real printer | Correct and legible | **HUMAN ONLY** | `human-only.spec.ts` |
...
| 6.7 | In admin, write a customer reply and close the case | Customer gets it by SMS and sees it on the receipt | **NOT COVERED** | — |
...
| # | Do this | Should happen | Coverage | Spec |
|---|---|---|---|---|
| 9.1 | **Restore your backup onto the test install** | Completes. Site works. Data is there | **HUMAN ONLY** | `human-only.spec.ts` |
| 9.2 | Check migrations ran cleanly during the restore | No errors | **HUMAN ONLY** | `human-only.spec.ts` |
| 9.3 | Confirm the queue worker is running | `php artisan queue:work redis` alive | **HUMAN ONLY** | `human-only.spec.ts` |
| 9.4 | Confirm cron is running | Scheduled tasks fire | **HUMAN ONLY** | `human-only.spec.ts` |
| 9.5 | Turn on maintenance mode, then off | Customers see the notice, then normal service | **HUMAN ONLY** | `human-only.spec.ts` |
...
| # | Do this | Should happen | Coverage | Spec |
|---|---|---|---|---|
| 10.1 | Order on a phone using **mobile data**, not wifi | Usable at real speeds | **HUMAN ONLY** | `human-only.spec.ts` |
| 10.2 | Have someone who has never used it place an order, and say nothing | They complete it without help | **HUMAN ONLY** | `human-only.spec.ts` |
| 10.3 | Run the POS for one full real shift | Open, sell, refund, close, with real staff | **HUMAN ONLY** | `human-only.spec.ts` |
...
| # | Test | Coverage | Spec |
|---|---|---|---|
| — | Real SMS arriving on a handset | **HUMAN ONLY** | `human-only.spec.ts` |

Local automation enables `sms_global_kill_switch` and expects `SMS_LIVE=false`. Demo/queued logs may exist; carrier delivery is never asserted.
