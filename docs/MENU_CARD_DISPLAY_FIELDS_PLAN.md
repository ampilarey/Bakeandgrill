...
Status: **Built.** Item fields such as `card_name` (and related card copy) are used in order-app `ProductCard` / `MenuViewPage` and admin editing paths. Body is design history for shipped card copy fields.
