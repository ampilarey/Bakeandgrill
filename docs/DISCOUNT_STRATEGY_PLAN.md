...
Status: **Built** (discount cards + evaluator path live). Admin `DiscountCardsPage`, discount-card models/migrations, and `PromotionEvaluator` handling exist. Strategy narrative below is the shipped design rationale; confirm any phased items still open before new work.

> Rescued from branch `claude/discount-strategy-plan` (not written fresh on this branch).
