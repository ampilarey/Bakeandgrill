...
Status: **Built.** Shared `prepareUpload` converts HEIC/HEIF before upload (admin + POS copies). Fix is live; body is the original diagnosis and approach.

> Rescued from branch `claude/iphone-media-upload-plan` (not written fresh on this branch).
