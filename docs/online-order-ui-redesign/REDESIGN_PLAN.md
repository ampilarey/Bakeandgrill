...
- `Category.image_url?: string | null` exists (`packages/shared/src/types/product.ts`) → rail icons; fallback = coloured circle with first grapheme (admin-driven names — no emoji map).
