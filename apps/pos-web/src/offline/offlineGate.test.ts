...
vi.mock("./db", () => ({
  loadCachedStaffSession: vi.fn(),
  loadCachedShift: vi.fn(),
  loadCachedMenu: vi.fn(),
}));

import { loadCachedMenu, loadCachedShift, loadCachedStaffSession } from "./db";

describe("offlineGate", () => {
  beforeEach(() => {
    localStorage.clear();
    vi.mocked(loadCachedStaffSession).mockReset();
  ...
  it("blocks when no cached shift", async () => {
    ...
    vi.mocked(loadCachedStaffSession).mockResolvedValue({
      id: "current",
      staff_user_id: 1,
      name: "Staff",
      permissions: [],
