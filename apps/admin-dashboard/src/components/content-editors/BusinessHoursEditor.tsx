...
export function BusinessHoursEditor({ label, description, value, onChange }: ContentEditorProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div style={{ background: 'var(--color-surface)', border: '1.5px solid var(--color-border)', borderRadius: 12, overflow: 'hidden' }}>
          ...
          return (
            <div key={key} style={{ display: 'flex', alignItems: 'center', padding: '10px 14px', borderTop: i === 0 ? 'none' : '1px solid var(--color-border-light)', gap: 12, background: isClosed ? '#FAFAFA' : 'var(--color-surface)' }}>
              ...
              <input
                ...
                style={{ flex: 1, height: 32, padding: '0 10px', border: '1px solid var(--color-border)', borderRadius: 8, fontSize: 13, fontFamily: 'inherit', outline: 'none', color: isClosed ? 'var(--color-text-muted)' : 'var(--color-text)', background: 'var(--color-surface)' }}
              />
            </div>
          );
        })}
