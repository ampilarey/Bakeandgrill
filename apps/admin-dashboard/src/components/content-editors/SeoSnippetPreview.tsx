...
function Counter({ value, soft }: { value: number; soft: number }) {
  ...
  return (
    <span style={{ fontSize: 11, fontWeight: 600, color: over ? '#B91C1C' : '#9C8E7E' }}>
      {value}/{soft}
    </span>
  );
}
    ...
    <div data-testid="seo-snippet" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
          <label style={{ fontSize: 12, fontWeight: 700, color: '#1C1408' }}>{titleLabel}</label>
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
          <label style={{ fontSize: 12, fontWeight: 700, color: '#1C1408' }}>{descriptionLabel}</label>
