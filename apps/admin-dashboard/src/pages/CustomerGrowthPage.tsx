    ...
    <>
      ...
      {tab === 'marketing' && (
        ...
        <Card>
          ...
          {automationMsg && (
            <p style={{ fontSize: 13, color: automationMsg.includes('saved') ? 'var(--color-success-strong)' : 'var(--color-danger-strong)', marginBottom: 12 }}>{automationMsg}</p>
          )}
          {!automation ? (
            <Spinner />
          ) : (
        ...
        <Card>
          ...
          {vipSettings ? (
            <div style={{ display: 'grid', gap: 12, maxWidth: 480 }}>
              ...
              {vipMsg && <p style={{ fontSize: 13, color: vipMsg.includes('saved') || vipMsg.includes('Synced') ? 'var(--color-success-strong)' : 'var(--color-danger-strong)', margin: 0 }}>{vipMsg}</p>}
            </div>
          ) : (
