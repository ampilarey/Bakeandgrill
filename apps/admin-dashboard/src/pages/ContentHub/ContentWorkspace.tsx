...
import type { GroupedBlocks } from './websiteFieldGroups';
import {
  WEBSITE_WORKSPACE,
  tabForGroup,
  type ContentWorkspaceConfig,
  type WorkspaceTab,
} from './contentWorkspaceConfig';

/**
 * Content & Branding — the page-tab workspace, shared by Website and Order App.
 *
 * Owner's layout, decided 2026-08-15 after three rejected drafts:
 *   • page tabs in a row, one per real page of the app;
 *   • Home is its ten sections, not fifty-odd loose settings;
 ...
 *   • the smaller pages are one plain form each, because at 13–22 settings
 *     nothing beats an ordinary well-spaced form.
 *
 * On a phone the tabs become a plain list — a row of five does not fit in 390px
 * without sideways scrolling, and a page you have to swipe to find is a page you
 * never open. Everything below that first screen is identical.
 *
 * Which tabs exist, and what each one holds, lives in contentWorkspaceConfig —
 * this file only knows how to draw them. The Order App reuses it verbatim
 * (owner, 2026-08-15: "Let's start order app").
 */
 ...
 */
function useHomeSectionVisibility(enabled: boolean, layoutRevision: number, app: ContentApp) {
  const [byType, setByType] = useState<Record<string, DeviceVisibility> | null>(null);
  const [version, setVersion] = useState(0);
  const [busyType, setBusyType] = useState<string | null>(null);
  const pageBlockApp = app === 'order_app' ? 'order_app' : 'website';

  useEffect(() => {
    if (!enabled) return;
    let cancelled = false;
    void fetchAdminPageBlocks(pageBlockApp, 'home')
      .then((res) => {
        if (cancelled) return;
  ...
  }, [enabled, layoutRevision, pageBlockApp]);
...
export type ContentWorkspaceProps = {
  /** Which tabs to draw and what each holds. Defaults to the website's five. */
  ...
  /** Phone: the pages are a list, and a page opens with a Back arrow. */
  isMobile?: boolean;
  /** Phone: leave the page and return to the list of pages. */
  onBack?: () => void;
};

export function ContentWorkspace({
  config = WEBSITE_WORKSPACE,
  loading,
  skeleton,
  sectionNames,
  activeGroup,
  ...
  const rootTestId = `${config.idPrefix}-content-workspace`;
  const selectedTab = tabForGroup(config, activeGroup);
  const activeTab: WorkspaceTab = selectedTab ?? config.tabs[0];
  const isHome = activeTab.kind === 'sections';
  /** Phone only: nothing picked yet, so show the list of pages. */
  const showPageList = isMobile && !selectedTab;
  ...
  /**
   * The tabs that have anything in them. A tab may cover several hub groups
   * (the Order App's nine small screens share one), so its count is the sum,
   * and the group it sends to the URL is the first of its own that exists.
   */
  const tabs = useMemo(() => {
    const present = new Set(sectionNames);
    return config.tabs
      .map((tab) => ({ tab, groups: tab.groups.filter((g) => present.has(g)) }))
      ...
      .map(({ tab, groups }) => ({
        ...
        group: groups[0],
        count: groups.reduce(
          (total, g) => total + blocksForContentView(g, contentBlocks, hubApp).length,
          0,
        ),
      }));
  }, [config, sectionNames, contentBlocks, hubApp]);

  const tabBlocks = useMemo(() => {
    const seen = new Set<string>();
    const out: ContentBlock[] = [];
    for (const group of activeTab.groups) {
      for (const block of blocksForContentView(group, contentBlocks, hubApp)) {
        if (seen.has(block.key)) continue;
        seen.add(block.key);
        out.push(block);
      }
    }
    return out.filter((b) => !isRedundantSeoDescription(b, contentBlocks));
  }, [activeTab, contentBlocks, hubApp]);

  const homeSections = useMemo(
    () => (activeTab.kind === 'sections' ? activeTab.buildSections(tabBlocks) : []),
    [activeTab, tabBlocks],
  );

  const pageGroups = useMemo(
    () => (activeTab.kind === 'form' ? activeTab.buildGroups(tabBlocks, hubApp) : []),
    [activeTab, tabBlocks, hubApp],
  );
  ...
  if (loading) {
    return <div className="wcw" data-testid={rootTestId}>{skeleton}</div>;
  }

  if (showPageList) {
    return (
      <div className="wcw wcw--mobile" data-testid={rootTestId} data-tab="">
        <p className="wcw-blurb" data-testid="wcw-blurb">{config.listIntro}</p>
        <div className="wcw-pagelist" data-testid="wcw-pagelist">
          {tabs.map((tab) => (
            <button
              ...
              onClick={() => onSelectGroup(tab.group)}
            >
              <span className="wcw-pagelist-main">
                <span className="wcw-pagelist-name">{tab.name}</span>
                <span className="wcw-pagelist-desc">{tab.tab.listBlurb}</span>
              </span>
    ...
    >
      {isMobile ? (
        <div className="wcw-mobile-head">
          ...
          <h2 className="wcw-mobile-title">{activeTab.label}</h2>
        </div>
      ) : (
        <div className="wcw-tabs" role="tablist" aria-label={config.tabsLabel}>
          {tabs.map((tab) => {
            const selected = tab.tab === activeTab;
            return (
              <button
                key={tab.name}
                type="button"
      ...
      <p className="wcw-blurb" data-testid="wcw-blurb">{activeTab.blurb}</p>
      ...
      {isHome ? (
        ...
        <div className="wcw-form" data-testid={`wcw-form-${activeTab.label}`}>
