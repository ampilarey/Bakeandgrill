  ...
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      {multiline ? (
        ...
        <input style={{ ...styles.input, borderColor: error ? "#dc3545" : "#EDE4D4" }}
          placeholder={placeholder} value={value} onChange={(e) => onChange(e.target.value)} />
      )}
...
function SummaryRow({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div style={styles.summaryRow}>
      <span style={{ color: highlight ? "#28a745" : "#5C4A2A" }}>{label}</span>
      <span style={{ color: highlight ? "#28a745" : "#5C4A2A", fontWeight: 500 }}>{value}</span>
    </div>
  );
}
            ...
            <>
              ...
              {loyaltyAccount && loyaltyAccount.points_balance > 0 && (
                <div style={styles.card}>
                  ...
                  <p style={{ fontSize: 14, color: "#5C4A2A", marginBottom: 12 }}>
                    You have <strong>{loyaltyAccount.points_balance} pts</strong> available (
                    <span style={{ color: "#D4813A" }}>MVR {laarToMvr(loyaltyAccount.points_balance)}</span> value).
                  </p>
...
// ── Styles ─────────────────────────────────────────────────────────────────────
const styles = {
  page:         { minHeight: "100vh", background: "#FFFDF9", fontFamily: "'Plus Jakarta Sans', sans-serif" } as React.CSSProperties,
  header:       { position: "sticky" as const, top: 0, background: "#fff", borderBottom: "1px solid #EDE4D4", padding: "12px 24px", display: "flex", alignItems: "center", gap: 16, zIndex: 100, boxShadow: "0 2px 8px rgba(0,0,0,0.04)" } as React.CSSProperties,
  backBtn:      { background: "none", border: "none", cursor: "pointer", color: "#D4813A", fontSize: 15, fontWeight: 600, padding: "4px 8px" } as React.CSSProperties,
  headerTitle:  { fontWeight: 700, fontSize: 18, color: "#1C1408", flex: 1 } as React.CSSProperties,
  ...
  card:         { background: "#fff", borderRadius: 16, padding: "20px 24px", boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid #EDE4D4" } as React.CSSProperties,
  sectionTitle: { fontSize: 16, fontWeight: 700, color: "#1C1408", margin: "0 0 16px", paddingBottom: 12, borderBottom: "1px solid #f0f0f0" } as React.CSSProperties,
  input:        { width: "100%", border: "1px solid #EDE4D4", borderRadius: 10, padding: "10px 12px", fontSize: 14, color: "#1C1408", outline: "none", boxSizing: "border-box" as const, marginBottom: 0, fontFamily: "inherit", transition: "border-color 0.15s" } as React.CSSProperties,
  fieldLabel:   { display: "block", fontSize: 13, fontWeight: 600, color: "#5C4A2A", marginBottom: 4 } as React.CSSProperties,
  ...
  typeBtn:      { flex: 1, padding: "12px 16px", border: "2px solid #EDE4D4", borderRadius: 12, background: "#fff", cursor: "pointer", fontSize: 14, fontWeight: 600, color: "#5C4A2A", transition: "all 0.15s" } as React.CSSProperties,
  typeBtnActive:{ borderColor: "#D4813A", background: "#e8f8fa", color: "#D4813A" } as React.CSSProperties,
  primaryBtn:   { background: "linear-gradient(135deg, #D4813A, #B86820)", color: "#fff", border: "none", borderRadius: 12, padding: "12px 24px", fontSize: 15, fontWeight: 700, cursor: "pointer", width: "100%", transition: "opacity 0.2s" } as React.CSSProperties,
  secondaryBtn: { background: "#fff", color: "#D4813A", border: "2px solid #D4813A", borderRadius: 10, padding: "10px 16px", fontSize: 14, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" as const } as React.CSSProperties,
  summaryRow:   { display: "flex", justifyContent: "space-between", fontSize: 14, marginBottom: 8 } as React.CSSProperties,
  totalRow:     { display: "flex", justifyContent: "space-between", fontWeight: 800, fontSize: 18, color: "#1C1408", borderTop: "2px solid #EDE4D4", paddingTop: 12, marginTop: 8 } as React.CSSProperties,
  ...
  checkboxLabel:{ display: "flex", alignItems: "center", fontSize: 14, color: "#1C1408", cursor: "pointer" } as React.CSSProperties,
