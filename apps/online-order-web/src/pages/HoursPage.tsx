  ...
  return (
    <div style={{ maxWidth: '640px', margin: '0 auto', padding: '3rem 1.5rem' }}>
      ...
      <div style={{
        ...
        <div>
          <p style={{ fontWeight: 700, color: '#1C1408', fontSize: '1rem', marginBottom: '0.2rem' }}>
            {isOpen === null ? 'Checking status…' : isOpen ? 'Open Now' : 'Currently Closed'}
          </p>
          {message && <p style={{ fontSize: '0.85rem', color: '#8B7355' }}>{message}</p>}
        </div>
      </div>
          ...
          return (
            <div
              ...
              style={{
                ...
                background: isToday ? 'rgba(212,129,58,0.06)' : i % 2 === 0 ? '#FFFDF9' : 'white',
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                ...
                <span style={{ fontWeight: isToday ? 700 : 500, color: isToday ? '#D4813A' : '#1C1408', fontSize: '0.925rem' }}>
                  {row.day}
                </span>
              </div>
              <span style={{ fontSize: '0.9rem', color: '#8B7355', fontWeight: isToday ? 600 : 400 }}>
                {row.hours}
              </span>
            </div>
          );
