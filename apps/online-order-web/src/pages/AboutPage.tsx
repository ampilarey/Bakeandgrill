  ...
  return (
    <div style={{ maxWidth: '860px', margin: '0 auto', padding: '3rem 1.5rem' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
        ...
        <h1 style={{ fontSize: 'clamp(1.75rem, 5vw, 2.75rem)', fontWeight: 800, color: '#1C1408', marginBottom: '1rem' }}>
          About Bake &amp; Grill
        </h1>
        <p style={{ fontSize: '1.1rem', color: '#8B7355', lineHeight: 1.7, maxWidth: '560px', margin: '0 auto' }}>
          Your favourite neighbourhood café in Malé — serving fresh bakes, hearty grills, and local flavours since day one.
        </p>
      </div>

      {/* Story */}
      <section style={{ background: '#FFFDF9', borderRadius: '20px', padding: '2.5rem', marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#1C1408', marginBottom: '1rem' }}>Our Story</h2>
        <p style={{ color: '#5C4A2A', fontSize: '1rem', lineHeight: 1.8, marginBottom: '1rem' }}>
        ...
        <p style={{ color: '#5C4A2A', fontSize: '1rem', lineHeight: 1.8 }}>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.25rem', marginBottom: '3rem' }}>
        {[
          { initial: 'F', color: '#D4813A', title: 'Fresh Ingredients', desc: 'We source locally and prep daily. No shortcuts, no compromises.' },
          { initial: 'C', color: '#D4813A', title: 'Community First', desc: 'We are part of Malé — we know our customers and they know us.' },
          { initial: 'Q', color: '#D4813A', title: 'Quality Always', desc: 'Whether it\'s a cup of tea or a full grill, everything gets the same care.' },
          { initial: 'B', color: '#D4813A', title: 'Built for Today', desc: 'Online ordering, table reservations, loyalty rewards — all in one place.' },
        ].map((v) => (
          <div
            key={v.title}
            style={{ background: 'white', border: '1px solid #EDE4D4', borderRadius: '14px', padding: '1.5rem', transition: 'all 0.2s' }}
          ...
          >
            ...
            <h3 style={{ fontSize: '0.975rem', fontWeight: 700, color: '#1C1408', marginBottom: '0.4rem' }}>{v.title}</h3>
            <p style={{ fontSize: '0.85rem', color: '#8B7355', lineHeight: 1.6 }}>{v.desc}</p>
          </div>
        ))}
      </section>

      {/* Location */}
      <section style={{ background: '#D4813A', color: 'white', borderRadius: '20px', padding: '2.5rem', textAlign: 'center', marginBottom: '2rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.875rem', justifyContent: 'center', flexWrap: 'wrap' }}>
          <Link
            to="/hours"
            style={{ background: 'white', color: '#D4813A', padding: '0.6rem 1.5rem', borderRadius: '999px', fontWeight: 600, fontSize: '0.9rem', textDecoration: 'none' }}
          >
            Opening Hours
          </Link>
          <Link
