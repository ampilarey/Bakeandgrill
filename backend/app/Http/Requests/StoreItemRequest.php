...
class StoreItemRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        if ($this->image_webp_url === '') {
            $this->merge(['image_webp_url' => null]);
        }
        if ($this->thumb_webp_url === '') {
            $this->merge(['thumb_webp_url' => null]);
        }
    ...
    public function rules(): array
    {
        return [
            ...
            'platter_groups.*.min_count' => 'nullable|integer|min:0|max:99',
            'platter_groups.*.max_count' => 'nullable|integer|min:0|max:99',
            'platter_groups.*.size_counts' => 'nullable|array',
            'platter_groups.*.size_counts.*' => 'integer|min:1|max:99',
