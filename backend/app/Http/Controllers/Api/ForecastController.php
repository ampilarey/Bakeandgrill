...
class ForecastController extends Controller
{
     ...
     */
    public function applySuggestedPreferredSuppliers(Request $request): JsonResponse
    {
        $validated = $request->validate([
            'item_ids' => ['required', 'array', 'min:1'],
            'item_ids.*' => ['integer', 'exists:inventory_items,id'],
        ...
        return response()->json($this->restock->applySuggestedPreferredSuppliers(
            $validated['item_ids'],
            (int) ($validated['lookback_days'] ?? 30),
            (int) ($validated['buy_lookback_days'] ?? 90),
            (int) ($validated['lead_days'] ?? 3),
    ...
    private function ywToMonday(int $yw): Carbon
    {
        $year = (int) floor($yw / 100);
        $week = $yw % 100;

        return Carbon::now()->setISODate($year, $week)->startOfWeek();
    }
