...
final class SignageAdminController extends Controller
{
    ...
    public function storeCampaign(Request $request): JsonResponse
    {
        $data = $request->validate([
            ...
            'windows.*.start' => 'nullable|date_format:H:i',
            'windows.*.end' => 'nullable|date_format:H:i',
    ...
    public function updateEmergency(Request $request): JsonResponse
    {
        ...
        $data = $request->validate([
            ...
            'entries.*.schedule.days.*' => 'integer|between:0,6',
            'entries.*.schedule.windows' => 'nullable|array',
            'entries.*.schedule.windows.*.start' => 'nullable|date_format:H:i',
    ...
    public function updateBanner(Request $request): JsonResponse
    {
        $data = $request->validate([
            ...
            'banners.*.schedule.days.*' => 'integer|between:0,6',
            'banners.*.schedule.windows' => 'nullable|array',
            'banners.*.schedule.windows.*.start' => 'nullable|date_format:H:i',
            'banners.*.schedule.windows.*.end' => 'nullable|date_format:H:i',
