...
<div class="doc-card">
    ...
    <form method="POST" action="{{ route('pos-pay.pay', $receipt->token) }}" style="margin-top:1.25rem;">
        @csrf
        <label style="display:flex;gap:0.625rem;align-items:flex-start;font-size:0.9rem;line-height:1.5;cursor:pointer;margin-bottom:1rem;">
            <input type="checkbox" name="accept_terms" value="1" id="accept-terms" style="margin-top:0.2rem;width:1.1rem;height:1.1rem;accent-color:var(--amber);" {{ old('accept_terms') ? 'checked' : '' }}>
            <span>
                I agree to the
                <a href="{{ url('/terms') }}" target="_blank" rel="noopener" style="color:var(--amber);text-decoration:underline;">Terms &amp; Conditions</a>,
                <a href="{{ url('/refund') }}" target="_blank" rel="noopener" style="color:var(--amber);text-decoration:underline;">Refund Policy</a>, and
                <a href="{{ url('/order/privacy') }}" target="_blank" rel="noopener" style="color:var(--amber);text-decoration:underline;">Privacy Policy</a>.
            </span>
        </label>
        @error('accept_terms')
            <div class="doc-alert doc-alert--error">{{ $message }}</div>
        @enderror
        <button type="submit" class="doc-btn doc-btn-primary" id="pay-btn" style="width:100%;" disabled>
